  ...
  return (
    <div className="theme-shell-card space-y-3 rounded-[1.5rem] p-4">
      <div className="space-y-1">
        ...
        {tooltip ? <p className="theme-shell-muted text-sm leading-6">{tooltip}</p> : null}
        {error ? <p className="text-sm text-destructive">{error}</p> : null}
      </div>
      <div className="grid gap-3 lg:grid-cols-2">
        {categories.map((category) => (
          <div key={category.id} className="theme-shell-card-strong rounded-[1.25rem] p-3">
            <div className="mb-3 flex items-center justify-between gap-3">
              <div className="flex min-w-0 items-center gap-3">
                <span
                  className="theme-shell-icon flex size-9 shrink-0 items-center justify-center rounded-full text-sm"
                ...
                <div className="min-w-0">
                  ...
                  <p className="theme-shell-soft text-sm">{category.color || 'Theme accent'}</p>
                </div>
              </div>
            </div>
            <Input
